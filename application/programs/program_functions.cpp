...
namespace GodotObjectCompiler {
  ...
  bool ProgramFunctions::copy_resources_to_folder(
      const Vector<String>& resource_glob_paths, const String& target_folder) {
    for (const String& copy_resources : resource_glob_paths) {
      for (const String& res_path : Resources::instance()->resources_recursive(copy_resources)) {
        ...
        auto file_path =
            path_concat(path_concat(target_folder, string_replace(copy_resources, "res://", "")), relative);
        if (!file_exists(file_path)) {
          String folder_path = path_base(file_path);
          if (!dir_exists(folder_path) && !create_dir_recursive(folder_path)) {
            return false;
          }
