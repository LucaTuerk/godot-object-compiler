...
namespace GodotObjectCompiler {
  ...
  bool FieldHandler::handles_node(const Ref<TreeSitterNode>& current_src) {
    return current_src->type_in({"field_declaration", "declaration", "function_definition", "expression_statement"});
  }

  ParserStep FieldHandler::handle(const Ref<TreeSitterNode>& current_src, Ref<Context>& current_target) {
