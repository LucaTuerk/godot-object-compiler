...
namespace GodotObjectCompiler {
  ...
  bool TypeHandler::handles_node(TSNode& node, const String& type) {
    return string_suffix(type, "type") || type == "qualified_identifier" ||
           type == "type_identifier" || type == "trailing_return_type";
  }

  NextStep TypeHandler::handle(ParserContext& context) {
    if (context.current_node->is<Class>()) {
    ...
    if (context.current_node->is<Type>()) {
      ...
      if (PlaceholderType* placeholder =
              context.current_node->find_child<PlaceholderType>()) {
        type_node = NodeDB::get_instance()->create<Type>();
