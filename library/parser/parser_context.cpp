...
namespace GodotObjectCompiler {
  ...
  String ParserContext::get_child_content(const TSNode& p_node,
                                          String p_child_name) {
  ...
  TSNode ParserContext::get_child_node(const TSNode& p_node,
                                       String p_field_name) {
    return ts_node_child_by_field_name(p_node, p_field_name.c_str(),
                                       p_field_name.length());
  }

  String ParserContext::copy_node_content(const TSNode& p_node) {
    return buffer.substr(ts_node_start_byte(p_node),
  ...
  ParserContext::ParserContext(const String& input) {
    ...
    tree = ts_parser_parse_string(parser, nullptr, buffer.c_str(),
                                  buffer.length());
