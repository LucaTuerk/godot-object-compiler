...
#define NODE_TYPE(type)                                                        \
 public:                                                                       \
  ...
  static inline bool __registered__##type##__ = DB::register_node_constructor( \
      #type, &GodotObjectCompiler::create<type>);
...
template <typename T, typename... Args>
T* GodotObjectCompiler::NodeDB::create(Args&&... args) {
  return create_with_id<T, Args...>(_generate_unique_id(),
                                    std::forward<Args>(args)...);
}

template <typename T, typename... Args>
T* GodotObjectCompiler::NodeDB::create_with_id(UID uid, Args&&... args) {
